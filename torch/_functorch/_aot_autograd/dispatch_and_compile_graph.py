...
def _root_module_for_non_strict_export(flat_fn):
...
def aot_dispatch_base_graph(
    ...
    *,
    ...
    non_strict_exporting_mod = _root_module_for_non_strict_export(flat_fn)
    if aot_config.is_export and non_strict_exporting_mod is not None:
        # in non-strict export
        ...
        def _map_assigned_buffer_to_proxy(_mod, name, buffer):
            # We intercept buffer assignments by registering this hook.
            ...
            if proxy_mode is not None and isinstance(
                ...
                # The value assigned to a buffer is a functional tensor, which wraps a
                # fake tensor, which in turn is associated with a proxy node.
                # Recall that we want to map the assigned buffer to this proxy node.
    ...
    if aot_config.is_export and non_strict_exporting_mod is not None:
        ...
        i = len(dict(non_strict_exporting_mod.named_parameters()))
        for name, _ in non_strict_exporting_mod.named_buffers():
