...
Tensor _clamp(
  ...
  if (v_self.is_quantized()) {
    float mini =
        min ? roundevenf(min->to<float>() / float(v_self.get_scale())) +
                float(v_self.get_zero_point())
            : -std::numeric_limits<float>::infinity();
    float maxi =
        max ? roundevenf(max->to<float>() / float(v_self.get_scale())) +
                float(v_self.get_zero_point())
            : std::numeric_limits<float>::infinity();
...
Tensor& _clamp_(
  ...
  if (v_self.is_quantized()) {
    float mini = min ? roundevenf(min->to<float>() / float(v_self.get_scale())) +
            float(v_self.get_zero_point())
                     : -std::numeric_limits<float>::infinity();
    float maxi = max ? roundevenf(max->to<float>() / float(v_self.get_scale())) +
            float(v_self.get_zero_point())
                     : std::numeric_limits<float>::infinity();
    const struct Block final {
      uvec3 extents;
